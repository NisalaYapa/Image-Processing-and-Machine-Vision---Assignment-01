...
# Show both images horizontally using Matplotlib
plt.figure(figsize=(12, 6))
plt.subplot(1, 2, 1)
plt.imshow(cv.cvtColor(large_merge_nn, cv.COLOR_BGR2RGB))
plt.title('Nearest Neighbor Interpolation')
plt.axis('off')

plt.subplot(1, 2, 2)
plt.imshow(cv.cvtColor(large_merge_bi, cv.COLOR_BGR2RGB))
